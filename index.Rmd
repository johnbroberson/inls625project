...
One additional way to depict this model's results is through the fitted values; I have coded a new `prediction` variable
...
ggplot(data, aes(x = sponsor_age, y = bill_len, shape = congress, color = logit_pred)) +
  geom_point() +
  facet_wrap(~ sponsor_party)


summarise(group_by(data, sponsor_age), pred = mean(logit_pred)) %>%
  ggplot(aes(x = sponsor_age, y = pred)) +
    geom_point()
...
Limitations: so much about politics can't be captured (see the intercept from the logit regression)

Going forward: 

* get 116th bills, use this best model to predict what'll happen. Trouble though: house majority switched
* expand to all bills *introduced* into the House, might be more variation in which get through, and definitely would be a much larger sample size
